...
import { getLocalDateISO, getStartOfWeek, getEndOfWeek, getStartOfMonth, getEndOfMonth } from '../../utils/dateUtils';
...
const TransactionPage: React.FC<TransactionPageProps> = ({
    ...
    const generatePDF = () => {
        const doc = new jsPDF();
        let title = 'Daily Transaction Report';
        let periodText = `Date: ${selectedDate}`;
        let filteredSales = sales.filter(s => s.timestamp.startsWith(selectedDate));
        let filteredExpenses = expenses.filter(e => e.date === selectedDate);
        let stockPurchases = dailyStats.record.stock_purchases;

        // Calculate attributes based on report type
        if (reportType === 'weekly') {
            const startStr = getStartOfWeek(selectedDate);
            const endStr = getEndOfWeek(selectedDate);
            title = 'Weekly Transaction Report';
            periodText = `Period: ${startStr} to ${endStr}`;

            filteredSales = sales.filter(s => {
                const d = s.timestamp.split('T')[0];
                return d >= startStr && d <= endStr;
            });
            filteredExpenses = expenses.filter(e => e.date >= startStr && e.date <= endStr);
            // Sum stock purchases for the week
            stockPurchases = dailyRecords
                .filter(r => r.date >= startStr && r.date <= endStr)
                .reduce((sum, r) => sum + r.stock_purchases, 0);

        } else if (reportType === 'monthly') {
            const startStr = getStartOfMonth(selectedDate);
            const endStr = getEndOfMonth(selectedDate);
            ...
            filteredExpenses = expenses.filter(e => e.date >= startStr && e.date <= endStr);
            // Sum stock purchases for the month
            stockPurchases = dailyRecords
                .filter(r => r.date >= startStr && r.date <= endStr)
                .reduce((sum, r) => sum + r.stock_purchases, 0);
        }
        ...
        const closingBalance = totalRevenue - stockPurchases - totalExpenses;
        ...
        autoTable(doc, {
            ...
            head: [['Metric', 'Amount']],
            body: [
                ['(+) Total Sales Revenue', format(totalRevenue)],
                ['(-) Total Stock Purchases', format(stockPurchases)],
                ['(-) Total Other Expenses', format(totalExpenses)],
                ['= Net Closing Balance', { content: format(closingBalance), styles: { fontStyle: 'bold', fillColor: [240, 255, 240] } }]
            ],
            theme: 'grid',
        ...
        if (filteredSales.length > 0) {
            ...
            autoTable(doc, {
                ...
                head: [['Date', 'Time', 'Product', 'Qty', 'Total']],
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                ...
                <div className="flex items-center gap-3">
                    <select
                        ...
                        className="p-2 border border-slate-200 rounded-xl font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                    >
